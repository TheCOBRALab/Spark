 ...
 * a lot of space.
 */
template <class key_t, class val_t> class SimpleMap : std::vector<std::pair<key_t, val_t>> {
    ...
    class {
      public:
        bool operator()(const key_val_t &x, const key_t &y) const { return x.first < y; }
    } comp;

    iterator binsearch(iterator first, iterator last, const key_t &key) {
    ...
    const_iterator binsearch(const_iterator first, const_iterator last, const key_t &key) const {
        first = std::lower_bound(first, last, key, comp);
        if (first == last || key < first->first) {
            return last;
        }
  ...
  public:
    ...
    bool exists(const key_t &key) const { return find(key) != key_val_vec_t::end(); }
    ...
    size_t size() const { return key_val_vec_t::size(); }

    size_t capacity() const { return key_val_vec_t::capacity(); }
