...
TraceArrows::TraceArrows(cand_pos_t n) : n_(n), ta_count_(0), ta_avoid_(0), ta_erase_(0), ta_max_(0) {}

TraceArrow &trace_arrow_from(TraceArrows &t, cand_pos_t i, cand_pos_t j) { return t.trace_arrow_[i].find(j)->second; }

bool exists_trace_arrow_from(TraceArrows &t, cand_pos_t i, cand_pos_t j) { return t.trace_arrow_[i].exists(j); }

void avoid_trace_arrow(TraceArrows &t) { t.ta_avoid_++; }
 ...
 * @param n New size
 */
void resize(TraceArrows &t, cand_pos_t n) { t.trace_arrow_.resize(n); }
...
cand_pos_t erasedT(TraceArrows &t) { return t.ta_erase_; }
cand_pos_t sizeT(TraceArrows &t) { return t.ta_count_; }
cand_pos_t avoidedT(TraceArrows &t) { return t.ta_avoid_; }
cand_pos_t maxT(TraceArrows &t) { return t.ta_max_; }