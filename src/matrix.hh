...
template <class T> class Matrix {
  ...
  protected:
     ...
     */
    size_type addr(size_type i, size_type j) const { return i * ydim_ + j; }
  ...
  public:
     ...
     *
     */
    Matrix(size_type xdim, size_type ydim, const elem_t *from = 0L) : mat_(xdim * ydim), xdim_(xdim), ydim_(ydim) {
        if (from != 0L) {
     ...
     */
    size_pair_type sizes() const { return size_pair_type(xdim_, ydim_); }
     ...
     * @param ydim second dimension
     */
    void resize(size_type xdim, size_type ydim, const T &__x) {
        xdim_ = xdim;
        ydim_ = ydim;
     ...
     * @return entry (i,j)
     */
    const elem_t &operator()(size_type i, size_type j) const { return mat_[addr(i, j)]; }
     ...
     */
    elem_t &operator()(size_type i, size_type j) { return mat_[addr(i, j)]; }
     ...
     * @return entry (i,j)
     */
    const elem_t get(size_type i, size_type j) const { return mat_[addr(i, j)]; }
     ...
     *
     */
    void set(size_type i, size_type j, const elem_t &x) { mat_[addr(i, j)] = x; }
     ...
     */
    template <class UnaryOperator> void transform(UnaryOperator f) { std::transform(mat_.begin(), mat_.end(), mat_.begin(), f); }
};
 ...
 */
template <class T> std::istream &operator>>(std::istream &in, Matrix<T> &mat) {
    typename Matrix<T>::size_pair_type sizes = mat.sizes();
